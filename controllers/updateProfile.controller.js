...
const updateProfile = async (req, res, next) => {
    try {
      ...
      if(userExists){
        ...
        await userSchema.updateOne({_id: userData._id}, {
        ...
        const newToken = jwt.sign({ exp: Math.floor(Date.now() / 1000) + 15 * 24 * 60 * 60, userId: newUserData._id, type, userData: newUserData }, 'secret key');
        res.json(newToken);
      }
    } catch (error) {
